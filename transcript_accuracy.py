...
def normalize_text(text_string, normalizations=['lower_case', 'standardize_numbers', 'expand_contractions', 'remove_punctuation', 'replace_hyphens', 'remove_spaces']):
    ...
    if 'standardize_numbers' in normalizations:
        modified_text = re.sub(r'\b(?P<num>\d+(,\d+)*(\.\d+)?)(st|nd|rd|th)\b', lambda m: num2words(float(m.group('num').replace(",", "")), to='ordinal'), modified_text)
        modified_text = re.sub(r'\b(?P<num>\d+(,\d+)*(\.\d+)?)\b', lambda m: num2words(float(m.group('num').replace(",", ""))), modified_text)
...
def demo_streamlit_app():
    ...
    default_gold_standard_text = 'Hey here\'s the thing that I was thinking about the  number 74 now.'
    default_asr_text = 'hey heres the thing what I was thinking-about the number 27 now'

    sidebar, main_body = st.columns([1,2])
    with sidebar:
        sidebar_container = st.container()
        col1, col2, col3 = sidebar_container.columns(3)
    with main_body:
        st.selectbox('Choose example transcripts to compare', ('Default short text', '2023 U.S. State of the Union speech (NYT vs. whisper-medium-en by OpenAI)', '2023 U.S. State of the Union speech (NYT vs. whisper-small-en by OpenAI)', '2023 U.S. State of the Union speech (NYT vs. whisper-tiny-en by OpenAI)', '2023 U.S. State of the Union speech (NYT vs. Nova by Deepgram)', 'The Bee Movie (2007) dialogue (gold standard vs. whisper-large-v2 by OpenAI)', 'The Bee Movie (2007) dialogue (gold standard vs. Nova by Deepgram)'), index=0, key='example_transcripts', help='Select a gold-standard transcript and an ASR transcript to compare it to', on_change=fill_example_transcripts, kwargs={'default_gold_standard': default_gold_standard_text, 'default_asr': default_asr_text}, disabled=False, label_visibility="visible")
        with st.form('transcript-form'):
            ...
            gold_standard_transcript = st.text_area('Type or paste a gold standard transcript here', value=default_gold_standard_text, help='Paste a gold standard transcript here. A gold standard transcript should ideally have been generated manually by a human in order to guarantee accuracy and thoroughness.', key='gold_standard_transcript', height=400)
            asr_transcript = st.text_area('Type or paste an ASR transcript here', value=default_asr_text, help='Paste an automatic speech recognition (ASR) transcript here. This transcript will be compared to the gold standard transcript in order to measure how accurate the ASR transcript was.', key='asr_transcript', height=400)
        ...
        if evaluate_button or st.session_state['run_comparison_automatically']:
            ...
            if normalized_text1 == normalized_text2:
                st.write('**The two normalized transcripts are identical.**')
            ...
            sidebar_container.subheader('Worst Offenders')
